...
string ::
  Chars
  -> Parser Chars
string cs = sequenceParser (is <$> cs)
...
hex ::
  Parser Char
hex = (handleError . readHex) <$> (thisMany 4 (satisfy isHexDigit))
  where
    handleError (Full x) = chr x
    handleError Empty = chr 0 -- This won't happen
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll ls = satisfy (\x -> and (ls <*> (pure x)))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny ls = satisfy (\x -> or (ls <*> (pure x)))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma c1 c2 pa = betweenCharTok c1 c2 (sepby pa (is ','))
