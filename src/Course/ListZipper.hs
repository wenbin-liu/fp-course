...
findLeft f (ListZipper (lx:.lxs) x r ) =
  if f lx == True then
    IsZ $ ListZipper lxs lx (x :. r)
  else
    findLeft f (ListZipper lxs lx (x :. r) )
...
findRight f (ListZipper l x (rx :. rxs)) =
  if f rx == True then
    IsZ $ ListZipper (x :. l) rx rxs
  else
    findRight f (ListZipper (x :. l) rx rxs)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (lx :. lxs) x r)=
  ListZipper lxs lx (x :. r)
moveLeftLoop (ListZipper Nil x r) =
  ListZipper l' x' Nil
  where
    rReverse = reverse (x :. r)
    x' = headOr x rReverse
    l' = drop 1 rReverse
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l x (lx:.lxs))=
  ListZipper (x:.l) lx lxs
moveRightLoop (ListZipper l x Nil) =
  ListZipper Nil x' r'
  where
    lReverse = reverse (x :. l)
    x' = headOr x lReverse
    r' = drop 1 lReverse
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (lx :. lxs) x r)=
  IsZ $ ListZipper lxs lx (x :. r)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l x (lx:.lxs))=
 IsZ $ ListZipper (x:.l) lx lxs
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (lx :. lxs) x r) =
  IsZ $  ListZipper (x :. lxs) lx r
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper l x (rx :. rxs)) =
  IsZ $  ListZipper l rx (x :. rxs)
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l x r)= extract $ moveRightN (length r) (ListZipper l x r)
  where
    extract (IsZ y) = y
...
moveLeftLoopN n lz
  | n == 0 = lz
  | n > 0 = moveLeftLoopN (n-1) (moveLeftLoop lz)
  | n < 0 = moveLeftLoopN (n+1) (moveRightLoop lz)

start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l x r)= moveLeftLoopN (length l) (ListZipper l x r)
...
deletePullLeft (ListZipper (lx :. lxs) _ r) = IsZ $ ListZipper lxs lx r
...
deletePullRight (ListZipper l _ (rx:.rxs)) = IsZ $ ListZipper l rx rxs
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x' (ListZipper l x r) = ListZipper (x :. l) x' r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x' (ListZipper l x r) = ListZipper l x' (x :. r)
...
instance Extend ListZipper where
  (<<=) f (ListZipper (lx :. lxs) x (rx :. rxs)) =
    ListZipper ((f (ListZipper lxs lx (x :. rx :. rxs))) :. lefts (f <<= (ListZipper lxs lx (x :. rx :. rxs))))
    (f $ ListZipper (lx :. lxs) x (rx :. rxs))
    ((f (ListZipper (x:.lx:.lxs) rx rxs)) :. rights (f <<= (ListZipper (x:.lx:.lxs) rx rxs)) )
  (<<=) f (ListZipper Nil x (rx :. rxs)) =
    ListZipper Nil
    (f $ ListZipper Nil x (rx :. rxs))
    ((f (ListZipper (x :. Nil) rx rxs)) :. rights (f <<= (ListZipper (x:. Nil) rx rxs)) )
  (<<=) f (ListZipper (lx :. lxs) x Nil) =
    ListZipper ((f (ListZipper lxs lx (x :. Nil))) :. lefts (f <<= (ListZipper lxs lx (x :. Nil))))
    (f $ ListZipper (lx :. lxs) x Nil)
    Nil
  (<<=) f x = ListZipper Nil (f x) Nil
...
instance Traversable ListZipper where
  traverse f lz = fromList <$> (pure lenLeft) <*> (traverse f (toList lz))
    where
    fromList n (x:.xs) = moveLeftLoopN (-n) (ListZipper Nil x xs)
    lenLeft = length $ lefts lz
...
instance Traversable MaybeListZipper where
  ...
  traverse f (IsZ x) = IsZ <$> (traverse f x)
