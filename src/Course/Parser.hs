...
(|||) (P px) (P py) = P $ \x ->
  let pxResult = px x
  in
    if (isErrorResult pxResult) == True then
      py x 
    else
      pxResult
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = (\x -> if (f x) then (pure x) else unexpectedCharParser x) =<< character
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser xs = foldRight f (valueParser Nil)  xs
  where f pa pb = (\x -> (x :. ) <$> pb) =<< pa
...
smokerParser ::
  Parser Bool
smokerParser = ((\_ -> valueParser True) =<< (is 'y')) |||
  ((\_ -> valueParser False) =<< (is 'n'))
...
phoneParser ::
  Parser Chars
phoneParser = lift3 (\x y _ -> x :. y) (digit) phoneBodyParser (is '#')
...
personParser ::
  Parser Person
personParser = (\x1 _ x3 _ x5 _ x7 _ x9 -> Person x1 x3 x5 x7 x9) <$> ageParser <*>  spaces1 <*>
  firstNameParser <*> spaces1 <*>  surnameParser <*>
  spaces1 <*> smokerParser <*> spaces1 <*> phoneParser 
